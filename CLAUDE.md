...
PROMERC — trazabilidad de compra-venta de desperdicio (cartón, etc.) por peso, operado desde báscula, para reemplazar el control en papel. Flujo central: **Pesaje** (tara + neto en báscula, con evidencia fotográfica y firmas digitales) → **Compra** (a un Proveedor, cada una genera su propio **Lote** — trazabilidad por operación, no agrupado por día) → **Venta** (a un Cliente, consumiendo uno o varios lotes) con validación automática de tolerancia entre lo comprado y lo reportado por el cliente, bloqueando el cierre si la diferencia excede el umbral hasta que un supervisor la apruebe.
...
- **Modelo de datos** (`prisma/schema.prisma`): `Pesaje` (dos tiempos: tara y neto, con `PesajeEmpaque` para conteo de pacas) → `Compra` (1:1 con `Pesaje`, N:1 con `Lote`) → `Lote` (folio `L-YYYYMMDD-###`, uno por compra/operación — `crearLoteParaCompra` en `src/lib/lote.ts`, corregible con auditoría; sin constraint único a nivel de DB porque lotes históricos de antes de este cambio agrupan varias compras, la garantía 1:1 es solo para compras nuevas) → `Venta` (vía `LoteMovimiento`, N:M con `Lote`). `Firma` y `Evidencia` son polimórficas (FK opcional a `Pesaje` o `Venta`, nunca ambas). `AuditLog` genérico para correcciones (folio de lote, aprobación de tolerancia).
...
- **OCR del ticket de báscula**: `src/lib/ocr.ts` (`leerTicketBascula`) manda la foto a Claude (`@anthropic-ai/sdk`, `claude-opus-4-8`, `output_config.format` con JSON schema) al seleccionar el archivo en `cerrar-pesaje-form.tsx`, y precarga peso/pesador/cliente/observaciones — el usuario revisa antes de enviar. Requiere `ANTHROPIC_API_KEY` (propia de console.anthropic.com, no la suscripción de Claude Code); sin ella la lectura falla y el formulario se llena a mano.
